...
_speaker_toggle_change(void *data, Evas_Object *obj, void *event_info)
{
	(void) data;
	(void) event_info;
	phoneui_utils_sound_speaker_set(elm_toggle_state_get(obj), NULL, NULL);
}
...
_mute_toggle_change(void *data, Evas_Object *obj, void *event_info)
{
	(void) data;
	(void) event_info;
	// FIXME: add a callback to check if it worked out
	phoneui_utils_sound_mute_set(FREE_SMARTPHONE_AUDIO_CONTROL_MICROPHONE,
					 elm_toggle_state_get(obj), NULL, NULL);
}
...
_volume_slider_change(void *data, Evas_Object *obj, void *event_info)
{
	...
	// FIXME: add a callback to check if it worked out
	phoneui_utils_sound_volume_set(FREE_SMARTPHONE_AUDIO_CONTROL_SPEAKER,
					   vol, NULL, NULL);
}
...
_mic_slider_change(void *data, Evas_Object *obj, void *event_info)
{
	...
	// FIXME: add a callback to check if it worked out
	phoneui_utils_sound_volume_set(FREE_SMARTPHONE_AUDIO_CONTROL_MICROPHONE,
					   vol, NULL, NULL);
}

static void
_volume_changed(void* _data, FreeSmartphoneAudioControl control, int volume)
{
	struct CallActiveViewData *data = (struct CallActiveViewData *)_data;
	switch (control) {
	case FREE_SMARTPHONE_AUDIO_CONTROL_SPEAKER:
		g_debug("new speaker volume is %d", volume);
		elm_slider_value_set(data->volume_slider, (double)volume);
		break;
	case FREE_SMARTPHONE_AUDIO_CONTROL_MICROPHONE:
		g_debug("new mic sensitivity is %d", volume);
		elm_slider_value_set(data->mic_slider, (double)volume);
		break;
	}
}

static void
_mute_changed(void* _data, FreeSmartphoneAudioControl control, gboolean mute)
{
	struct CallActiveViewData *data = (struct CallActiveViewData *)_data;
	if (control == FREE_SMARTPHONE_AUDIO_CONTROL_MICROPHONE) {
		g_debug("Mic-Mute changed: value %d", mute);
		elm_toggle_state_set(data->mute_toggle, mute);
	}
}

static void
_device_changed(void* _data, FreeSmartphoneAudioDevice device)
{
	struct CallActiveViewData* data = _data;
	g_debug("Audio Device changed to %s",
		 free_smartphone_audio_device_to_string(device));
	elm_toggle_state_set(data->speaker_toggle,
...
call_active_view_show(struct Window *win, GHashTable * options)
{
	...
	phoneui_info_register_and_request_audio_device(_device_changed, data);
	phoneui_info_register_and_request_audio_volume(_volume_changed, data);
	phoneui_info_register_and_request_audio_mute(_mute_changed, data);
...
call_active_view_hide(struct CallActiveViewData *data)
{
	...
	phoneui_info_unregister_audio_device(_device_changed, data);
	phoneui_info_unregister_audio_volume(_volume_changed, data);
	phoneui_info_unregister_audio_mute(_mute_changed, data);
