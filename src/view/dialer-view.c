...
frame_dialer_show(void *_data)
{
	...
	data->keypad =
		(Evas_Object *) elm_keypad_add(window_evas_object_get(win));
	...
	edje_object_signal_callback_add(window_layout_get(win), "click",
					"number", frame_dialer_number_clicked,
					data);
	edje_object_signal_callback_add(window_layout_get(win), "mouse_down",
					"delete",
