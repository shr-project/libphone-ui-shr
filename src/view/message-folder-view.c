						  ...
						  void *_data)
{
	...
	evas_object_smart_callback_add(data->bt1, "clicked",
				       messages_button_new_clicked, win);
	...
	evas_object_smart_callback_add(data->bt2, "clicked",
				       messages_button_show_clicked, win);
...
*/
	...
	Etk_Scrolled_View *scrolled_view =
		etk_tree_scrolled_view_get(ETK_TREE(data->tree_folders));
	...
	Etk_Tree_Row *row =
		etk_tree_row_append(ETK_TREE(data->tree_folders), NULL,
				    data->col1, parameters, NULL);
	...
	window_swallow(win, "list_swallow",
