...
profiles_fill(Evas_Object *profiles)
{
	...
	while(profile_list[idx] != NULL) {
		hsi = elm_hoversel_item_add(profiles, profile_list[idx], NULL,
					    ELM_ICON_NONE, on_phone_profile_set,
					    profile_list[idx]);
		idx++;
	}
...
{
	Evas_Object *win, *box, *netbox, *profiles, *buttons, *button;
	Evas_Object *toggle, *frame, *label;
...
on_connectivity_settings(void *data, Evas_Object *obj, void *event_info)
{
	...
	Evas_Object *netbox, *button, *check, *frame, *subframe, *networks;
	Evas_Object *icon, *entry, *table;
...
{
	Evas_Object *win, *toolbar, *icon;
	Elm_Toolbar_Item *tb_phone, *tb_item;
        int ret;

	g_debug("Initializing the quick settings screen");
        ret = ui_utils_view_init(VIEW_PTR(view), ELM_WIN_BASIC,
				 D_("Quick settings"), NULL, NULL, _quick_settings_destroy_cb);
	...
	elm_icon_standard_set(icon, "edit");
	evas_object_show(icon);
	tb_phone = elm_toolbar_item_add(toolbar, icon, D_("Phone"),
					on_phone_settings, NULL);

	icon = elm_icon_add(win);
	elm_icon_standard_set(icon, "edit");
	evas_object_show(icon);
	tb_item = elm_toolbar_item_add(toolbar, icon, D_("Connectivity"),
				       on_connectivity_settings, NULL);
